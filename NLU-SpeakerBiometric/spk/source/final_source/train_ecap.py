...
trainer = pl.Trainer( max_epochs=20,log_every_n_steps =200,accelerator='gpu',check_val_every_n_epoch =1,callbacks=[ScoreCallback(),pl.callbacks.ModelCheckpoint(args.save_path,filename ="{epoch}-{VEER:.3f}-{mindcf:.3f}",save_last =True,mode ='min',monitor ="VEER",save_on_train_epoch_end =True)])
