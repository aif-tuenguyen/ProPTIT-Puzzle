...
from src.scorings.plda.plda_base import PLDA
class CORALPlus(object):
    ...
    def plda_read(self,plda):
        with open(plda,"rb") as f:
            self.mean = np.load(f).reshape(-1,1)
            self.dim = self.mean.shape[0]
            self.within_var = np.load(f)
            self.within_var =self.within_var.reshape(self.dim, self.dim)
            self.between_var = np.load(f).reshape(self.dim, self.dim)

    def plda_write(self,plda):
        with open(plda,"wb") as f:
            np.save(f, self.mean)
            np.save(f,self.within_var.reshape(-1,1))
            np.save(f,self.between_var.reshape(-1,1))
